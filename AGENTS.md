...
網站風格是 Apple 官網式的極簡白灰介面：大標題、中央對齊 Hero、充足留白、簡潔 panel、柔和陰影與膠囊 CTA。修改 UI 時維持：

- 純 HTML/CSS 與 Google Fonts Inter。
- 清楚的 section 分隔與 RWD。
- 約 `0.3s` 的 hover / transition。
- 表格與表單優先可讀性。
- 不加入大型框架、深色高對比背景、過度裝飾或雜亂卡片堆疊。
...
收入類型目前只允許 `rec center` 作為分類，收入來源只允許：
